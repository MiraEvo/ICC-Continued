    ...
    public partial class MainWindow : Window {
        ...
        public static class ShapeDrawingHelper {
            ...
            public static double CaculateRotateAngleByGivenTwoPoints(Point firstPoint, Point lastPoint) {
                var vec1 = new double[] { lastPoint.X - firstPoint.X, lastPoint.Y - firstPoint.Y };
                ...
                if (isDrawTop && isDrawBottom) {
                    ...
                    if (isDrawBottom)
                        for (double r = 0; r <= Math.PI; r += 0.01)
                            pointList.Add(new Point(0.5 * (st.X + ed.X) + a * Math.Cos(r), 0.5 * (st.Y + ed.Y) + b * Math.Sin(r)));
                    if (isDrawTop)
                        for (var r = Math.PI; r <= 2 * Math.PI; r += 0.01)
                            pointList.Add(new Point(0.5 * (st.X + ed.X) + a * Math.Cos(r), 0.5 * (st.Y + ed.Y) + b * Math.Sin(r)));
                }
                return pointList;
            }
            ...
            public static Stroke GenerateLineStroke(Point st, Point ed, DrawingAttributes drawingAttrs) {
                var pointList = new List<Point> { new Point(st.X, st.Y), new Point(ed.X, ed.Y) };
                var point = new StylusPointCollection(pointList);
                return new Stroke(point) { DrawingAttributes = drawingAttrs.Clone() };
            }
            ...
            public static StrokeCollection GenerateDashedLineStrokeCollection(Point st, Point ed, DrawingAttributes drawingAttrs) {
                ...
                for (var i = 0.0; i < d; i += step * 2.76) {
                    ...
                    var stroke = new Stroke(point) {
                        DrawingAttributes = drawingAttrs.Clone()
                    };
                    strokes.Add(stroke);
                }
                return strokes;
