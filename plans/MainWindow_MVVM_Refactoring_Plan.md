...
**最后更新**: 2026-01-03 14:21

### 已完成任务
- [x] 阶段一：MVVM 基础设施搭建
- [x] 阶段二：Settings 系统 MVVM 化
- [x] 阶段三：创建核心服务层
- [x] 阶段四：MainWindow ViewModel 拆分
- [x] 阶段五：View 层绑定重构（核心工具栏）
  - [x] 5.1 设置 MainWindow.xaml 的运行时 DataContext
  - [x] 5.2 核心工具按钮命令绑定清理（移除双重执行问题）
    - 已迁移按钮：光标、画笔、橡皮擦（线擦/板擦）、套选、清空、撤销、重做
  - [x] 5.3 热键系统委托到 ViewModel 命令（Ctrl+Z/Y/E, Alt+D/Q/S/E/B/C/V/L）
  - [x] 5.4 添加工具状态的双向绑定（选中状态高亮）
    - 添加 `FloatingBarButton.IsSelected` 附加属性
    - 绑定 5 个核心工具按钮的选中状态
...
### 待处理任务
- [ ] 5.5 将设置面板的控件绑定到 SettingsViewModel（延后 - 需要更多迭代）
- [ ] 6.1 验证应用程序正常启动
- [ ] 6.2 验证工具切换功能正常
- [ ] 6.3 验证撤销/重做功能正常
- [ ] 6.4 验证设置保存和加载正常
